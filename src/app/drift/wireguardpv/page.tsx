...
const WireguardPV= () => {
  const [WireguardPVData, setWireguardPVData] = useState(null)
  ...
  useEffect(() => {
    client
      .fetch(`*[_type == "drift" && title == "Wireguard"]`)
      .then((data) => setWireguardPVData(data))
      .catch(console.error)
  }, [])

  if (!WireguardPVData) return <div>Loading...</div>
  ...
  return (
    <div className='flex bg-dark w-screen flex-col min-h-screen  '>
      ...
      <div className='text-gray-50 flex justify-center flex-col mt-24 w-screen text-center px-12'>
        <h1 className='text-4xl text-gray-50 font-serif'>Wireguard</h1>
        <div className='flex justify-center items-center'>
        {WireguardPVData.map((item: any, index: number) => (
