...
const NextPV= () => {
  const [NextPVData, setNextPVData] = useState(null)
  ...
  useEffect(() => {
    client
      .fetch(`*[_type == "utvikling" && title == "NextJS"]`)
      .then((data) => setNextPVData(data))
      .catch(console.error)
  }, [])

  if (!NextPVData) return <div>Loading...</div>
  ...
  return (
    <div className='flex bg-dark w-screen flex-col min-h-screen  '>
      ...
      <div className='text-gray-50 flex justify-center flex-col mt-24 w-screen text-center px-12'>
        <h1 className='text-4xl text-gray-50 font-serif'>Next JS</h1>
        <div className='flex justify-center items-center'>
        {NextPVData.map((item: any, index: number) => (
